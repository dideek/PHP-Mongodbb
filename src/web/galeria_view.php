...
<div id="main">
  ...
  echo '<form method="POST">';
    for($page;$page<=$ilosc_stron;$page++){
        ...
        $fotki=$db->temporary->find([],$opts);
        foreach($fotki as $zdjecie){
            if(in_array($zdjecie['_id'],$_SESSION['cart'])){
                echo '<input type="checkbox" name="koszyk[]" value="'.$zdjecie['_id'].'" checked>';
            }
            else echo '<input type="checkbox" name="koszyk[]" value="'.$zdjecie['_id'].'">';
    ...
    echo '<input type="submit" value="Dodaj wybrane do ulubionych">';
    echo '</form>';
  ?>
